...
const CODE_ZIP_FILE = {
  ...
  'nodejs6.10': fs.readFileSync(path.join(__dirname, 'lambda-nodejs6.10.zip')),
};
...
class LambdaRunner extends Runner {
  ...
  create(params, cb) {
    ...
    lambda.createFunction(params.createParams, (err, data) => {
      params.createReturn = {
        err: err,
        data: data
      };
  ...
  update(params, cb) {
    ...
    lambda.updateFunctionConfiguration(params.updateParams, (err, data) => {
      params.updateReturn = {
        err: err,
        data: data
      };
  ...
  processAliases(params, cb) {
    ...
    async.forEachOf(object.aliases, function (alias, key, next) {
      lambda.getAlias({FunctionName: object.name,Name: alias}, (err, data) => {
        if (err && 404 == err.statusCode) {
          lambda.createAlias({FunctionName:object.name,FunctionVersion:'$LATEST',Name:alias}, (err, data) => {
